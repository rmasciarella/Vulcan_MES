"""User management API routes with enterprise-grade type safety."""
...
def read_users(session: SessionDep, skip: int = 0, limit: int = 100) -> UsersPublic:
    ...
    Returns:
        UsersPublic response with user data and count
    """
    """
    Retrieve users.
    """
...
def read_user_by_id(
    ...
    Returns:
        User public data if found and authorized

    Raises:
        HTTPException: If user doesn't have privileges
    """
    """
...
def delete_user(
    ...
    # Note: This would delete related items, but Item model is not imported
    # If Item relationships exist, they should be handled via CASCADE or explicit deletion
    session.delete(user)
    session.commit()
    return Message(message="User deleted successfully")
